...
## 7) Migrar dados existentes do Supabase para MongoDB

Se você já tem conteúdo no Supabase e quer levar tudo para MongoDB:

1. No `backend/.env`, configure também:
...
2. Instale dependências do backend (se ainda não fez):
...
3. Garanta que sua API Mongo esteja rodando (`npm run dev`) e que o admin exista.
...
- Migra as coleções: `projetos`, `projeto_galeria`, `autores`, `posts`, `depoimentos`
- Recria o vínculo da galeria com os novos IDs dos projetos no Mongo
- Preserva timestamps e campos de negócio

### Importante

- `RESET_TARGET=true` limpa os dados atuais do Mongo antes de importar.
- Em primeira migração completa, recomenda-se usar `RESET_TARGET=true`.
- Use `SUPABASE_SERVICE_ROLE_KEY` (não anon key), para evitar bloqueio por RLS.

## 8) Segurança recomendada (produção)
